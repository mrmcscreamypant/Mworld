...
var Rapier3dComponent = TaroEventingClass.extend({
    classId: 'Rapier3dComponent',
    componentId: 'physics',
    init: function (entity, options, callback, physicsComponent) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this._gravity = { x: 0.0, y: -9.81, z: 0.0 };
            try {
                yield RAPIER.init();
                callback();
            }
    ...
    gravity: function (x, y) {
        ...
        return { x: this._gravity.x, y: this._gravity.z };
    },
    sleep: function (val) {
        if (val !== undefined) {
            this._sleep = val;
    ...
    createBody: function (entity, body, isLossTolerant) {
        ...
        const position = { x: entity._translate.x / this._scaleRatio, y: 6.05, z: entity._translate.y / this._scaleRatio };
        ...
        const rigidBodyDesc = new RAPIER.RigidBodyDesc(bodyType).setTranslation(position.x, position.y, position.z);
        ...
        rigidBody.setRotation(this._angleToQuaternion(angle));
        rigidBody.setEnabledRotations(false, true, false, false);
        rigidBody.setEnabledTranslations(true, false, true, false);
                ...
                if (fixtureDef.shape) {
                    ...
                    switch (fixtureDef.shape.type) {
                        case 'rectangle': {
                            ...
                            colliderDesc = RAPIER.ColliderDesc.cuboid(width, 0.1, height).setTranslation(x, 0, y);
                            break;
                        }
                        case 'circle': {
                            ...
                            colliderDesc = RAPIER.ColliderDesc.cylinder(0.1, radius, 0).setTranslation(x, 0, y);
                            break;
                        }
                    }
    ...
    getEntitiesInRegion: function (region) {
        let entities = [];
        let shape = new RAPIER.Cuboid(region.width / this._scaleRatio / 2, 0.2, region.height / this._scaleRatio / 2);
        let shapePos = { x: region.x / this._scaleRatio, y: 6.05, z: region.y / this._scaleRatio };
    ...
    start: function (continuousPhysics) {
        ...
        let groundColliderDesc = RAPIER.ColliderDesc.cuboid(1000.0, 6, 1000.0);
        ...
        this._world.bodies.forEach((body) => {
            ...
            if (!body.isFixed() && !body.isSleeping()) {
                if (entity && !entity._stats.isHidden) {
                    ...
                    // var angle = -Math.asin(body.rotation().y) * 2 * (180 / Math.PI);
                    var angle = 0;
    ...
    applyForce: function (entity, x, y) {
        ...
        if (!body)
            return;
        body.addForce({ x: x, y: 0, z: y }, true);
    },
    applyImpulse: function (entity, x, y) {
        ...
        if (!body)
            return;
        body.applyImpulse({ x: x, y: 0, z: y }, true);
    },
    ...
    translateTo: function (entity, x, y) {
        ...
        if (!body)
            return;
        body.setTranslation({ x, y: 6.05, z: y }, true);
    },
    rotateTo: function (entity, angle) {
        ...
        if (!body)
            return;
        body.setRotation(this._angleToQuaternion(angle), true);
    },
    ...
    raycastLine(start, end) {
        ...
        dir.x /= length;
        dir.y /= length;
        let ray = new RAPIER2D.Ray({ x: start.x, y: 6.05, z: start.y }, { x: dir.x, y: 0, z: dir.y });
        let solid = true;
    ...
    _angleToQuaternion: function (angle, axis = { x: 0, y: 1, z: 0 }) {
...
if (typeof module !== 'undefined' && typeof module.exports !== 'undefined') {
    module.exports = Rapier3dComponent;
}
//# sourceMappingURL=Rapier3dComponent.js.map